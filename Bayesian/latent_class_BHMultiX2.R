...
update_alpha_q <- function(alpha = alpha_q, params = c(2,2)){
  ...
  log_dens <- function(alpha_k, Q_k){# unnormalized
    ...
    n*log(gamma(sum(alpha))) - n*log(gamma(alpha_k)) + (a - 1)*alpha_k - 
      b*alpha_k + alpha_k*sum(log(Q_k))
  }
  alpha0 <-  alpha
...
# run the sampler
w <- c(.14, .12, .15) # tuning parameters
nchains <- 1
nburn <- 84
niters <- 101142
nthin <- 27
...
for (chain in 1:nchains){
  ...
  for (burn in 1:nburn){
    ...
    P <- update_P()
    Q <- update_Q()
    alpha_q_draw <- update_alpha_q()
    alpha_q <- alpha_q_draw[1:3]
    accept <- accept + alpha_q_draw[4:6]
  }
